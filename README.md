...
Пакет SQaLice предназначен для гибкой обработки запросов к БД на PostgreSQL в проектах с применением go-swagger. SQaLice позволяет запрашивать конкретный набор полей из установленного __target__, фильтровать, сортировать и ограничивать получаемый набор строк.
...
Для оператора *==* возможно указание нескольких значений черех запятую, без пробелов. В таком случае происходит отбор значения по переданному массиву.
